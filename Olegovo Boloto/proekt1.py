...
class Board:
    ...
    def on_click(self, cell_coords):
        if sboard[int(cell_coords[0])][int(cell_coords[1])] == 1:
            pygame.draw.rect(screen, pygame.Color('green'), (self.left + self.cell_size * int(cell_coords[0]) + 1 + 700,
                                                             self.top + self.cell_size * int(cell_coords[1]) + 1,
                                                             self.cell_size - 2,
                                                             self.cell_size - 2), 0)
    ...
    while True:
        try:
            ...
            if sms[0] == 'Вы 1'.encode():
                ...
                data[0] = 3
            elif 'Стрельнул' in sms[0].decode():
                sboard[int(sms[0].decode().split()[1])][int(sms[0].decode().split()[2])] = 1
                board.on_click((sms[0].decode().split()[1], sms[0].decode().split()[2]))
        except OSError:
            continue
